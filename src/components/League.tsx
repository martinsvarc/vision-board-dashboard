...
const mockLeagueData: LeagueData[] = [
  {
    ...
    profile_image:
      "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/agent45.jpg",
    badges: [
  ...
  },
  {
    ...
    created_at: "2024-02-20",
    profile_image:
      "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/agent23.jpg",
    badges: [
  ...
  },
  {
    ...
    created_at: "2024-02-20",
    profile_image:
      "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/agent35.jpg",
    badges: [
...
export default function League() {
  const [category, setCategory] = React.useState<"daily" | "weekly" | "monthly">(
    "daily"
  );
  ...
  const getBorderColor = (rank: number, isCurrentUser: boolean) => {
    ...
    switch (rank) {
      case 1:
        return "border-[#fbb351]";
      case 2:
        return "border-[#546bc8]";
      case 3:
        return "border-[#fb9851]";
      default:
  ...
  const renderUserProfile = (user: LeagueData & { rank: number }) => {
    ...
    return (
      <div
        key={user.id}
        className={`flex items-center gap-2 p-2 rounded-[20px] shadow-sm bg-white border-2 ${borderColor}`}
      >
    ...
    <Card className="bg-white shadow-lg h-full">
      <CardContent className="p-3 h-full flex flex-col">
        <div className="flex flex-col gap-3 mb-4">
          <h2 className="text-[25px] font-bold text-[#556bc7] font-montserrat text-center">
            League
          </h2>
          <Tabs
            value={category}
            onValueChange={(value) =>
              setCategory(value as "daily" | "weekly" | "monthly")
            }
            className="w-full"
          >
            <TabsList className="w-full grid grid-cols-3 bg-gray-50/50 p-1.5 rounded-full">
              <TabsTrigger
                value="daily"
                className="relative z-10 rounded-full px-3 py-1.5 text-xs sm:text-sm font-medium text-gray-600 transition-all duration-300 hover:text-gray-900 data-[state=active]:bg-white data-[state=active]:shadow"
              >
                Daily League
              </TabsTrigger>
              <TabsTrigger
        ...
        <div className="flex-1 flex flex-col gap-3 min-h-0 overflow-hidden">
          ...
          <h3 className="text-lg font-semibold text-[#556bc7] font-montserrat pl-2">
            Top 3 places
          </h3>
          <div className="space-y-2 overflow-y-auto custom-scrollbar pr-1">
            {leagueData
              ...
              .map((user, index) =>
                renderUserProfile({ ...user, rank: index + 1 })
              )
              .slice(0, 3)}
          </div>
        </div>
