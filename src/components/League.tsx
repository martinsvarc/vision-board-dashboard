...
const renderUserProfile = (user: LeagueData & { rank: number }) => {
  ...
  return (
    <div key={user.id} className={`flex items-center gap-2 p-2 rounded-[20px] shadow-sm bg-white border-2 ${borderColor}`}>
        ...
        <div className="flex items-center gap-1.5 text-sm font-medium text-gray-900 truncate">
          ...
          <div className="flex-shrink-0 flex items-center">
            {user.badges?.map((badge, index) => (
              <img key={index} src={badge} alt={`Badge ${index + 1}`} className="w-4 h-4 object-contain" />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="h-[220px] w-full bg-gray-100 rounded-[20px] p-2 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fill: '#333', fontSize: 12 }} dy={2} />
              <YAxis axisLine={false} tickLine={false} tick={{ fill: '#333', fontSize: 12 }} dx={-5} />
              ...
              <Area type="monotone" dataKey="topUserPoints" stroke="#fbb350" strokeWidth={2} fillOpacity={0.3} fill="url(#goldGradient)" dot={false} stackId="1" />
              <Area type="monotone" dataKey="userPoints" stroke="#51c1a9" strokeWidth={2} fillOpacity={1} fill="url(#colorPoints)" dot={false} stackId="2" />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        <div className="flex-1 flex flex-col gap-3 min-h-0 overflow-hidden">
          {renderUserProfile({ ...currentUserProfile, rank: currentUserProfile.rank ?? 0 })}
